#import "BusinessBeanCopyUtil.h"
#import <objc/runtime.h>
#import "BusinessBean.h"
#import "ViewModel.h"
#import "NSDictionary+Extensions.h"

#pragma mark----- deep copy collections
...
id cloneBusinessObject(id inObj);
...
NSArray *cloneArray(NSArray *inArr)
{
    ...
    for (id element in inArr) {
        id newObj = element;
        if ([element isKindOfClass:[BusinessBean class]]) {
            newObj = cloneBusinessObject(element);
...
NSDictionary *cloneDictionary(NSDictionary *inDict)
{
    ...
    for (id aKey in allKeys) {
        ...
        if (newObj != NULL) {
            [retDict setSafeObject:newObj forKey:aKey];
        }
    }
...
id cloneBusinessObject(id item)
{
        ...
        for (const Ivar *p = ivars; p < ivars + numberOfIvars; p++) {
            ...
            if (oldValue != nil) {
                ...
                if ([typeStr hasPrefix:@"@\""] && typeStr.length > 3) {
                    ...
                    if ([className isEqualToString:NSStringFromClass([NSString class])]) {
                    ...
                    } else if ([oldValue isKindOfClass:[BusinessBean class]] || [oldValue isKindOfClass:[ViewModel class]]) {
                        newValue = cloneBusinessObject(oldValue);
                        ...
                        NSLog(@"Error: unsupport class type==%@, member==%@", className, key);
                    }
                }
...
@implementation BusinessBeanCopyUtil

+ (id)cloneItem:(id)item
{
    return cloneBusinessObject(item);
}
